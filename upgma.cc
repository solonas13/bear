...
unsigned int upgma_sim2 ( TPOcc ** D, unsigned int d, struct TSwitch sw, int * R )
{
...
unsigned int upgma_dist2 ( TPOcc ** D, unsigned int d, struct TSwitch sw, int * R )
{
	/* This is the original distanc matrix */
	double ** M;
        M = ( double ** ) calloc ( d , sizeof ( double * ) );
...
unsigned int upgma_sim ( TPOcc ** D, unsigned int d, struct TSwitch sw, int * R, unsigned char ** seq )
{
	/* This is the original distanc matrix */
	double ** M;
        M = ( double ** ) calloc ( d , sizeof ( double * ) );
	...
	while ( dd > 1 )
	{
		...
		double * score = ( double * ) calloc ( nE[jmax] , sizeof ( double ) );
		for ( int i = 0; i < nE[imax]; i ++ )
		{
			unsigned int ii = C[imax][i];
		...
		{
			unsigned int ii = C[imax][i];
			unsigned int jj = C[jmax][best];
			int rho = D[ii][jj] . rot - ( R[ii] - R[jj] );

			if ( D[ii][jj] . err >= sw . min_sim )
			{
				if ( rho > 0 )
					R[ii] += rho % strlen ( ( char * ) seq[ii] );
				else
					R[ii] += ( strlen ( ( char * ) seq[ii] ) + rho );
			}
			else
				R[ii] = -1;
		}
...
unsigned int upgma_dist ( TPOcc ** D, unsigned int d, struct TSwitch sw, int * R, unsigned char ** seq )
{
	/* This is the original distanc matrix */
	double ** M;
        M = ( double ** ) calloc ( d , sizeof ( double * ) );
	...
	while ( dd > 1 )
	{
		...
		double * errors = ( double * ) calloc ( nE[jmin] , sizeof ( double ) );
		for ( int i = 0; i < nE[imin]; i ++ )
		{
			unsigned int ii = C[imin][i];
			...
			{
				unsigned int jj = C[jmin][j];
				errors[j] += D[ii][jj] . err;
			}
		}

		unsigned int best;
		double min_errors = DBL_MAX;
		for ( int j = 0; j < nE[jmin]; j ++ )
		{
			errors[j] = errors[j] / nE[jmin]; 
			if ( errors[j] <= min_errors )
			{
				min_errors = errors[j];
				best = j;
			}
